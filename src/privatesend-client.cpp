    ...
    if(strCommand == NetMsgType::DSQUEUE) {
        ...
        if(dsq.fReady) {
            ...
            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- Cloaking queue (%s) is ready on masternode %s\n", dsq.ToString(), infoMn.addr.ToString());
            ...
            LogPrint("privatesend", "DSQUEUE -- new Cloaking queue (%s) from masternode %s\n", dsq.ToString(), infoMn.addr.ToString());
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSFINALTX -- message doesn't match current Cloaking session: nSessionID: %d  nMsgSessionID: %d\n", nSessionID, nMsgSessionID);
            return;
        }
        ...
        if(nSessionID != nMsgSessionID) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Cloaking session: nSessionID: %d  nMsgSessionID: %d\n", nSessionID, nMsgSessionID);
            return;
        }
...
{
    if(fMasterNode) {
        LogPrintf("CPrivateSendClient::SendDenominate -- Cloaking from a Masternode is not supported currently.\n");
        return false;
    }

    if(txMyCollateral == CMutableTransaction()) {
        LogPrintf("CPrivateSendClient:SendDenominate -- Cloaking collateral not set\n");
        return false;
    }
    ...
    if(!CheckDiskSpace()) {
        ...
        LogPrintf("CPrivateSendClient::SendDenominate -- Not enough disk space, disabling Cloaking.\n");
        return false;
    }
...
bool CPrivateSendClient::DoAutomaticDenominating(CConnman& connman, bool fDryRun)
{
    ...
    if(WaitForAnotherBlock()) {
        LogPrintf("CPrivateSendClient::DoAutomaticDenominating -- Last successful Cloaking action was too recent\n");
        strAutoDenomResult = _("Last successful Cloaking action was too recent.");
        return false;
    }
...
bool CPrivateSendClient::SubmitDenominate(CConnman& connman)
{
    ...
    for(int i = nPrivateSendRounds; i > 0; i--) {
        if(PrepareDenominate(i - 1, i, strError, vecTxInRet, vecTxOutRet)) {
            LogPrintf("CPrivateSendClient::SubmitDenominate -- Running Cloaking denominate for %d rounds, success\n", i);
            return SendDenominate(vecTxInRet, vecTxOutRet, connman);
        }
        LogPrint("privatesend", "CPrivateSendClient::SubmitDenominate -- Running Cloaking denominate for %d rounds, error: %s\n", i, strError);
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    if(PrepareDenominate(0, nPrivateSendRounds, strError, vecTxInRet, vecTxOutRet)) {
        LogPrintf("CPrivateSendClient::SubmitDenominate -- Running Cloaking denominate for all rounds, success\n");
        return SendDenominate(vecTxInRet, vecTxOutRet, connman);
    }

    // Should never actually get here but just in case
    LogPrintf("CPrivateSendClient::SubmitDenominate -- Running Cloaking denominate for all rounds, error: %s\n", strError);
...
bool CPrivateSendClient::PrepareDenominate(int nMinRounds, int nMaxRounds, std::string& strErrorRet, std::vector<CTxIn>& vecTxInRet, std::vector<CTxOut>& vecTxOutRet)
{
    ...
    if (GetEntriesCount() > 0) {
        strErrorRet = "Already have pending entries in the Cloaking pool";
        return false;
    }
...
void ThreadCheckPrivateSendClient(CConnman& connman)
{
    ...
    // Make this thread recognisable as the Cloaking thread
