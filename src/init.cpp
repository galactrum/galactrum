...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "galactrum (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-cloakingmultisession=<n>", strprintf(_("Enable multiple Cloaking mixing sessions per block, experimental (0-1, default: %u)"), DEFAULT_PRIVATESEND_MULTISESSION));
    strUsage += HelpMessageOpt("-cloakingrounds=<n>", strprintf(_("Use N separate masternodes for each denominated input to mix funds (2-16, default: %u)"), DEFAULT_PRIVATESEND_ROUNDS));
    strUsage += HelpMessageOpt("-cloakingamount=<n>", strprintf(_("Keep N ORE anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to Cloaking by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), DEFAULT_PRIVATESEND_LIQUIDITY));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Galactrum Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        mapArgs["-enablecloaking"] = "1";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -enablecloaking=1\n", __func__, nLiqProvTmp);
        mapArgs["-cloakingrounds"] = "99999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -cloakingrounds=99999\n", __func__, nLiqProvTmp);
        mapArgs["-cloakingamount"] = "999999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -cloakingamount=999999\n", __func__, nLiqProvTmp);
        mapArgs["-cloakingmultisession"] = "0";
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Galactrum version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Galactrum.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Galactrum is shutting down."));
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Galactrum is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Galactrum is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Galactrum") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Galactrum to complete") << "\n";
    ...
    privateSendClient.nPrivateSendRounds = std::min(std::max((int)GetArg("-cloakingrounds", DEFAULT_PRIVATESEND_ROUNDS), 2), privateSendClient.nLiquidityProvider ? 99999 : 16);
    privateSendClient.nPrivateSendAmount = std::min(std::max((int)GetArg("-cloakingamount", DEFAULT_PRIVATESEND_AMOUNT), 2), 999999);
