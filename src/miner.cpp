...
// ***TODO*** ScanHash is not yet used in Galactrum
//
...
void static BitcoinMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        // Throw an error if no script was provided.  Ths can happen
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    int cons = connman.GetNodeCount(CConnman::CONNECTIONS_ALL);
                    if (!fvNodesEmpty && !IsInitialBlockDownload() && masternodeSync.IsSynced())
                        break;
                    MilliSleep(5000);
                } while (true);
            }
            LogPrintf("GalactrumMiner -- creating new block");
            //
            // Create new block
            //
            ...
            int64_t nTargetTimespan = 30 * chainparams.GetConsensus().nPowTargetSpacing;
            LogPrintf("params.nPowTargetTimespan = %d    nActualTimespan = %d\n", nTargetTimespan, nActualTimespan);
            if (!pblocktemplate.get())
            {
                LogPrintf("GalactrumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
