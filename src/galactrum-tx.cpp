...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = strprintf("%s bitcoin-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  galactrum-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n" +
              "  galactrum-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n" +
              "\n";
