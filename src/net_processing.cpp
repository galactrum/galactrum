...
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    ...
    int64_t activeTipBlockTime = tip->GetBlockTime();
    int64_t adjustedTime = GetAdjustedTime();
    int64_t compareTime = adjustedTime - consensusParams.nPowTargetSpacing * 20;
    //return chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - consensusParams.nPowTargetSpacing * 20;
    LogPrintf("NetProcessing - CanDirectFetch adjustedTime %" PRIu64 "\n", adjustedTime);
    LogPrintf("NetProcessing - CanDirectFetch activeTipBlockTime: %" PRIu64 " compareTime %" PRIu64 "\n", activeTipBlockTime, compareTime);
    return activeTipBlockTime > compareTime;
}
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    {
        LogPrintf("NetProcessing::HEADERS !\n");
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            if (nCount == 0) {
                LogPrintf("NetProcessing::HEADERS No interesting headers, count was zero\n");
