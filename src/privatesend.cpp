...
void CPrivateSendBase::CheckQueue()
{
    ...
    while(it != vecDarksendQueue.end()) {
        if((*it).IsExpired()) {
            LogPrint("cloaking", "CPrivateSendBase::%s -- Removing expired queue (%s)\n", __func__, (*it).ToString());
...
bool CPrivateSend::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    for (const auto& txin : txCollateral.vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            LogPrint("cloaking", "CPrivateSend::IsCollateralValid -- Unknown inputs in collateral transaction, txCollateral=%s", txCollateral.ToString());
            return false;
        }
    ...
    if(nValueIn - nValueOut < GetCollateralAmount()) {
        LogPrint("cloaking", "CPrivateSend::IsCollateralValid -- did not include enough fees in transaction: fees: %d, txCollateral=%s", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("cloaking", "CPrivateSend::IsCollateralValid -- %s", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState validationState;
        if(!AcceptToMemoryPool(mempool, validationState, MakeTransactionRef(txCollateral), false, NULL, NULL, false, maxTxFee, true)) {
            LogPrint("cloaking", "CPrivateSend::IsCollateralValid -- didn't pass AcceptToMemoryPool()\n");
            return false;
        }
    }
...
std::string CPrivateSend::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_DENOM:                 return _("No matching denominations found for cloaking.");
...
void CPrivateSend::CheckDSTXes(int nHeight)
{
    ...
    LogPrint("cloaking", "CPrivateSend::CheckDSTXes -- mapDSTX.size()=%llu\n", mapDSTX.size());
}
...
void CPrivateSend::SyncTransaction(const CTransaction& tx, const CBlockIndex *pindex, int posInBlock)
{
    ...
    LogPrint("cloaking", "CPrivateSendClient::SyncTransaction -- txid=%s\n", txHash.ToString());
}
