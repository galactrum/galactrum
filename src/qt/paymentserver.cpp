...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("galactrum://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'galactrum://' is not a valid URI. Use 'galactrum:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // Galactrum: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
