...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"address\"          (string) The account galactrum address\n"
            "\nExamples:\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The galactrum address to be associated with an account.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "                             The recipient will receive less amount of Galactrum than you enter in the amount field.\n"
...
UniValue listaddressbalances(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"address\": amount,       (string) The galactrum address and the amount in " + CURRENCY_UNIT + "\n"
            "  ,...\n"
            "}\n"
            "\nExamples:\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "\nResult:\n"
            "\"address\"         (string) A galactrum address associated with the keys.\n"
...
void ListTransactions(const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const COutputEntry& s, listSent)
        {
            ...
            entry.push_back(Pair("category", (it != wtx.mapValue.end() && it->second == "1") ? "cloaked" : "send"));
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            "walletpassphrase \"passphrase\" timeout ( cloakingonly )\n"
            ...
            "3. cloakingonly          (boolean, optional, default=false) If is true sending functions are disabled.\n"
            "\nNote:\n"
            ...
            "\nUnlock the wallet for 60 seconds but allow Cloaking only\n"
    ...
    if (fForMixingOnly && !pwalletMain->IsLocked(true) && pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLET_ALREADY_UNLOCKED, "Error: Wallet is already unlocked for cloaking only.");
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            + (!fLiteMode ?
            "  \"cloaked_balance\": xxxxxx, (numeric) the anonymized ORE balance of the wallet in " + CURRENCY_UNIT + "\n" : "") +
    ...
    if(!fLiteMode)
        obj.push_back(Pair("cloaked_balance",       ValueFromAmount(pwalletMain->GetAnonymizedBalance())));
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid galactrum address");
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "walletpassphrase",         &walletpassphrase,         true,   {"passphrase","timeout","cloakingonly"} },
