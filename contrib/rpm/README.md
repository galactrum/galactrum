...
The RPM spec file provided here is for Galactrum 0.12.0 and builds on CentOS
...
reference the release tarball available from https://galactrum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Galactrum git repository](https://github.com/galactrum/galactrum) but are not part of
...
Currently only `Source10` is used, for the example `galactrum.conf` file.
...
in the galactrum git repository. At present only an SVG version of the Galactrum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the galactrum
...
needed when 0.12.1 is released, a proper fix is already in the Galactrum git
master branch.
...
idea to build Galactrum against a static version of that library compiled
...
The `galactrum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `galactrum-server` package. At this point, most RPM based
...
The `galactrum-server` package should have SELinux support. How to properly do
...
Galactrum against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/galactrum/). He has been
packaging Galactrum for Fedora at least since 2012.
...
4. I renamed the `galactrum` package that contains the Qt GUI to `galactrum` as
that appears to be how the general population refers to it, in contrast to
`galactrum-xt` or `galactrum-classic`. I wanted to make sure the general population
