...
Below are some notes on how to build Galactrum for Windows.

The options known to work for building Galactrum on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Galactrum Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Galactrum applications.
...
Note that for WSL the Galactrum source path MUST be somewhere in the default mount file system, for
example /usr/src/galactrum, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\galactrum`, for example:

    make install DESTDIR=/mnt/c/workspace/galactrum
...
causes two of the galactrum executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the galactrum code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the galactrum source code).
