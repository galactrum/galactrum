...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Galactrum wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Galactrum.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Galactrum block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ORE they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ORE being spent is exactly
...
- **Increased security for multisig:** Galactrum addresses (both P2PKH addresses
  ...
  believed to be provided by Galactrum's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Galactrum paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Galactrum's current ledger (called the UTXO set),
  ...
  note that Galactrum does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Galactrum users to individually opt-in to almost any change in the Galactrum
  Script language when those users receive new transactions.  Features
  currently being researched by Galactrum contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Galactrum community members are studying several other improvements
...
Since Galactrum 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/galactrum/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in galactrumconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.galactrumstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libgalactrumconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_galactrum by itself (luke-jr)
...
- #8829 `2a8bca4` Add galactrum-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` galactrum-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_galactrumds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name galactrum.qrc  (ORESG)
...
- #8817 `bcf3806` update galactrum-tx to output witness data (jnewbery)
