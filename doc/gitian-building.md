...
*Setup instructions for a Gitian build of Galactrum using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Galactrum
...
- [Building Galactrum](#building-galactrum-core)
...
Clone the git repositories for Galactrum and Gitian.
...
in the Galactrum repository under 'Fetch and create inputs' to install sources which require
...
To build Galactrum (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Galactrum repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/galactrum/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/galactrum/galactrum
...
For example:
```bash
URL=https://github.com/crowning-/galactrum.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit galactrum=${COMMIT} --url galactrum=${URL} ../galactrum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit galactrum=${COMMIT} --url galactrum=${URL} ../galactrum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit galactrum=${COMMIT} --url galactrum=${URL} ../galactrum/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the galactrum git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../galactrum/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/galactrum
SIGPATH=/some/root/path/galactrum-detached-sigs

./bin/gbuild --url galactrum=${BTCPATH},signature=${SIGPATH} ../galactrum/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/galactrum-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/galactrum-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/galactrum-osx-build.assert
```
